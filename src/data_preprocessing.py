"""
Data preprocessing script for sign language detection.
Processes images from raw dataset and extracts MediaPipe keypoints.
"""
...
def get_sorted_image_files(action_path):
    ...
    Sorts numerically (1.jpg, 2.jpg, ..., 10.jpg) instead of alphabetically.
    
    Args:
        action_path: Path to action folder containing images
    ...
    try:
        # Try to sort numerically (assumes filenames like '0.jpg', '1.jpg', etc.)
        sorted_files = sorted(image_files, key=lambda f: int(os.path.splitext(f)[0]))
    except ValueError:
        # If numerical sorting fails, use alphabetical
        print(f"  ⚠ Could not sort files numerically. Using alphabetical sort.")
        print(f"    Ensure filenames are like '0.jpg', '1.jpg', etc.")
...
def create_output_directories():
    ...
    print(f"✓ Created directories for {len(ACTIONS)} actions × {NO_SEQUENCES} sequences")


def process_images_to_sequences(visualize=False, limit_actions=None):
    """
    Process raw images and extract keypoints using MediaPipe.
    
    Args:
        visualize: If True, display images with landmarks (slower)
        limit_actions: List of specific actions to process (None = all)
        
    Returns:
        Dictionary with processing statistics
    """
    # Filter actions if limit specified
    ...
    print(f"Total sequences to create: {len(actions_to_process) * NO_SEQUENCES}")
    print(f"Total frames to process: {len(actions_to_process) * NO_SEQUENCES * SEQUENCE_LENGTH}")
    print("=" * 70)
    ...
    with HolisticDetector(
        ...
        for action in actions_to_process:
            ...
            if len(sorted_files) < required_images:
                ...
                print(f"    Will process only {len(sorted_files) // SEQUENCE_LENGTH} complete sequences")
            ...
            for sequence_num in tqdm(range(actual_sequences), 
                                    desc=f"  Sequences for '{action}'",
                                    ncols=70):
                ...
                for frame_num, filename in enumerate(sequence_files):
                    ...
                    # Extract keypoints
                    keypoints = extract_keypoints_holistic(results)
                    ...
                    # Visualize if requested (slows down processing significantly)
                    if visualize:
                        from src.utils.mediapipe_utils import draw_styled_landmarks
    ...
    stats['frames_per_second'] = stats['total_frames'] / stats['duration'] if stats['duration'] > 0 else 0
...
def print_statistics(stats):
    ...
    print("=" * 70)
    print(f"Total actions processed: {stats['total_actions'] - stats['skipped_actions']}/{stats['total_actions']}")
    print(f"Total sequences created: {stats['total_sequences']}")
    print(f"Total frames processed: {stats['total_frames']}")
    print(f"Failed frames: {stats['failed_frames']}")
    print(f"Duration: {stats['duration']:.2f} seconds ({stats['duration']/60:.2f} minutes)")
