...
def load_sequences(
    data_path=None, actions=None, no_sequences=None, sequence_length=None
):
    ...
    if no_sequences is None:
        ...
        if os.path.exists(first_action_path):
            no_sequences = len(
                [
                    d
                    for d in os.listdir(first_action_path)
                    if os.path.isdir(os.path.join(first_action_path, d))
                ]
            )
            print(f"ℹ Auto-detected {no_sequences} sequences per action from disk")
        else:
            no_sequences = NO_SEQUENCES
            print(f"⚠ Could not auto-detect, falling back to config: {no_sequences}")
    ...
    print(
        f"Expected: {len(actions)} actions × {no_sequences} sequences = {len(actions) * no_sequences} total"
    )
    print("-" * 70)
    ...
    for action in actions:
        ...
        for sequence_idx in range(no_sequences):
            ...
            if len(frame_files) != sequence_length:
                print(
                    f"  ⚠ '{action}' seq {sequence_idx}: {len(frame_files)} frames, expected {sequence_length}"
                )
                continue
            ...
            for frame_num in range(sequence_length):
                ...
                if not os.path.exists(frame_path):
                    print(
                        f"  ⚠ '{action}' seq {sequence_idx}: Missing frame {frame_num}"
                    )
                    break
        ...
        print(
            f"  ✓ '{action}': Loaded {action_sequences_found}/{no_sequences} sequences"
        )
...
def split_data(X, y, test_size=None, random_state=None, stratify=True):
    ...
    try:
        X_train, X_test, y_train, y_test = train_test_split(
            X, y, test_size=test_size, random_state=random_state, stratify=y_labels
        )
...
def verify_data_integrity(
    ...
    for action in actions:
        action_stats = {"complete": 0, "incomplete": 0, "missing": 0}
        ...
        print(
            f"{status} '{action}': {action_stats['complete']}/{no_sequences} complete "
